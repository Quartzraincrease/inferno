...
// inferno-next event-delegation listeners are scoped to that target (matching
...
describe('portal — event delegation', () => {
  ...
  it('unmounting the entire root detaches portal-target listeners', () => {
    ...
    // to detect whether the (now-detached) inferno-next listener still runs.
    (stray as any).$$click = () => { leakedHandlerFired = true; };
    stray.click();
